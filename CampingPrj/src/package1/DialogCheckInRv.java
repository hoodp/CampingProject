...
/**********************************************************************
 * The following class is used for creating a new RV site. This class 
 ...
 *
 *********************************************************************/
public class DialogCheckInRv extends JDialog implements
ActionListener {
	...
	/** JTextField where user enters name*/
	private JTextField txtName;

	/** JTextField where user enters stay length */
	private JTextField txtStayLength;

	/** ComboBox displaying available sites */
	private JComboBox cmbSites;

	/** ComboBox displaying available power */
	private JComboBox cmbPower;

	/** Allows user to select a date */
	private JDateChooser choose;

	/** String array of available powers */
	 ...
	 *****************************************************************/
	public DialogCheckInRv(JFrame parent, RV d, ArrayList<Site> sites){
		super(parent, true);
		unit = d;
		result = CANCEL;
		...
		// openings prevents the user from selecting a site that is 
		// already taken 
		openings = availSites();
		cmbSites = new JComboBox();
		
		// fills the items in the combobox
		...
		// the min is 5 years before today
		choose.setMinSelectableDate(setBounds(-5,choose.getDate()));
		
		// max is 5 years from today
		...
		power = new String[] { "30", "40", "50" };
		lblPower = new JLabel("Power needed:");
		lblPower.setBounds(10, 187, 192, 14);
		panel.add(lblPower);
		cmbPower = new JComboBox();
		cmbPower.setModel(new DefaultComboBoxModel(power));
		cmbPower.setBounds(10, 205, 192, 20);
		panel.add(cmbPower);
		getContentPane().add(panel, BorderLayout.CENTER);
		...
		// submits form when enter button is pressed
		...
		// sets JDialog to middle of screen
	...
	/******************************************************************
	 * 
	 * @param field JTextField to check for error
	...
	private boolean checkInts(JTextField field) {
		try {
			...
			// days staying
			int length = Integer.parseInt(field.getText());
			
			// day is more than 5 years from now
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (button == btnOK) {
			...
			if (valid()) {
				...
				// adds the estimated number of days staying the check
				// in date.
				chkOut.add(Calendar.DAY_OF_MONTH, unit.daysStaying);
				unit.checkOut = chkOut;
				unit.siteNumber = Integer.parseInt(cmbSites.
						getSelectedItem().toString());
				unit.setPower(Integer.parseInt(cmbPower.
						getSelectedItem().toString()));
				result = OK;
				
				// Used to display the estimated cost
